...
import { type PivotRound } from "@/lib/games/miniGames";
...
export function PivotPuzzle() {
  ...
  const check = () => {
    setChecked(true);
    const ok = eq(computePivot(round, groupBy, valueField, agg), round.expect);
    dispatch({ type: "recordGameAttempt", game: "pivot_puzzle", level: n, passed: ok });
    if (ok) dispatch({ type: "recordGameScore", game: "pivot_puzzle", level: n, score: n });
  };
